...
void Room::enter(Player& player) {
    ...
    switch (type) {
        ...
        case ENTRANCE:
            std::cout << "This is the entrance to the dungeon." << std::endl;
            break;
            
        case EXIT:
            std::cout << "You found the exit! You can now leave the dungeon." << std::endl;
            break;
...
void Room::display() const {
    ...
    std::cout << "\nAvailable exits: ";
    if (northOpen) std::cout << "North(W) ";
    if (southOpen) std::cout << "South(S) ";
    if (eastOpen) std::cout << "East(D) ";
    if (westOpen) std::cout << "West(A) ";
    std::cout << "\n";
}
...
void Room::trapEffect(Player& player) {
    ...
    // Display trap activation with animation
    ...
    // Assign appropriate key for the action
    char keyToPress = avoidAction[0]; // First letter of the action
